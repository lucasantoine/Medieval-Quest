...
public class Garde implements IEvenement {
	...
	private static void seduire(Personnage p) {
		...
		if(R.nextInt(2)==1) {
			...
			Saisie.pressEnter("Vous perdez 5 points de vie.", false);
	...
	private static void payer(Personnage p) {
		...
		if(p.getArg()>=20) {
			...
			Saisie.pressEnter("Vous perdez 5$.", false);
			p.getHud().getStats()[3].add(-5);
		} else {
	...
	private static void intimider(Personnage p) {
		...
		if(p.getAtk()>=20) {
			...
			Saisie.pressEnter("Vous gagnez 10$.", false);
	...
	private static void fuir(Personnage p) {
		...
		Saisie.pressEnter("Vous perdez 10 points de vie.", false);
